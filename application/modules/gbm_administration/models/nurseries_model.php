...
{	
	...
	*
	*/
	public function get_all_nurseries($table, $where, $per_page, $page, $order = 'county_name', $order_method = 'ASC')
	{
		//retrieve all users
		$this->db->from($table);
		$this->db->select('*');
	...
	*/
	public function add_county()
	{
		...
		if($this->db->insert('nurseries', $data))
		{
			return TRUE;
		}
		else{
	...
	*/
	public function update_county($county_id)
	{
		...
		if($this->db->update('nurseries', $data))
		{
			return TRUE;
		}
		else{
	...
	*/
	...
	{
		if($this->db->delete('nurseries', array('county_id' => $county_id)))
		{
			return TRUE;
		}
		else{
